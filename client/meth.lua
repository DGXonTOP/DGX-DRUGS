...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processChemicals')
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processMeth')
...
RegisterNetEvent('DGX-DRUGS:ProcessChemicals', function()
...
RegisterNetEvent('DGX-DRUGS:ChangeTemp', function()
			...
			if result.ret then
				exports['ps-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('DGX-DRUGS:cancelProcessing')
						TriggerServerEvent('DGX-DRUGS:processFailUp')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('DGX-DRUGS:ChangeTemp2', function()
			...
			if result.ret then
				exports['ps-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('DGX-DRUGS:cancelProcessing')
						TriggerServerEvent('DGX-DRUGS:processFailDown')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('DGX-DRUGS:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
RegisterNetEvent('DGX-DRUGS:EnterLab', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('DGX-DRUGS:ExitLab', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
