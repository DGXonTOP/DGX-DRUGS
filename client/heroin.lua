...
local function ProcessHeroin()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processPoppyResin')
...
RegisterNetEvent('DGX-DRUGS:ProcessPoppy', function()
...
RegisterNetEvent("DGX-DRUGS:processHeroin",function()
	QBCore.Functions.TriggerCallback('DGX-DRUGS:validate_items', function(result)
		if result.ret then
			ProcessHeroin()
		else
...
RegisterNetEvent("DGX-DRUGS:pickHeroin", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
			...
			TriggerServerEvent('DGX-DRUGS:pickedUpPoppy')
			isPickingUp = false
