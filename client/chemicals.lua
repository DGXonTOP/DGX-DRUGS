...
local function createChemicalMenu()
    local chemMenu = {
        ...
        },
        {
			...
			params = {
                event = "DGX-DRUGS:hydrochloric_acid",
            }
        },
        {
            ...
            txt = Lang:t("menu.chemicals"),
			params = {
                event = "DGX-DRUGS:sodium_hydroxide",
            }
        },
        {
...
RegisterNetEvent('DGX-DRUGS:chemicalmenu', createChemicalMenu)
...
local function process_hydrochloric_acid()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processHydrochloric_acid')
...
local function process_lsa()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:process_lsa')
...
local function process_sulfuric_acid()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processprocess_sulfuric_acid')
...
local function process_sodium_hydroxide()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processsodium_hydroxide')
...
RegisterNetEvent("DGX-DRUGS:hydrochloric_acid", function()
...
RegisterNetEvent("DGX-DRUGS:lsa", function()
    QBCore.Functions.TriggerCallback('DGX-DRUGS:validate_items', function(result)
		if result then
			process_lsa()
		else
...
RegisterNetEvent("DGX-DRUGS:sulfuric_acid", function()
    QBCore.Functions.TriggerCallback('DGX-DRUGS:validate_items', function(result)
		if result then
...
RegisterNetEvent("DGX-DRUGS:sodium_hydroxide", function()
    QBCore.Functions.TriggerCallback('DGX-DRUGS:validate_items', function(result)
		if result then
...
RegisterNetEvent("DGX-DRUGS:pickChemicals", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.pickup_chemicals"), 10000, false, true, {
			...
			TriggerServerEvent('DGX-DRUGS:pickedUpChemicals')
			isPickingUp = false
