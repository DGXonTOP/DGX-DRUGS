...
local function ProcessCoke()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processCocaLeaf')
...
local function CutCokePowder()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processCocaPowder')
...
local function ProcessBricks()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('DGX-DRUGS:processCocaBrick')
...
RegisterNetEvent('DGX-DRUGS:ProcessCocaFarm', function()
...
RegisterNetEvent('DGX-DRUGS:ProcessCocaPowder', function()
	local coords = GetEntityCoords(PlayerPedId())
	local amount = 10
	local amount2 = 5
...
RegisterNetEvent('DGX-DRUGS:ProcessBricks', function()
...
RegisterNetEvent('DGX-DRUGS:EnterCWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('DGX-DRUGS:ExitCWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('DGX-DRUGS:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('DGX-DRUGS:pickedUpCocaLeaf')
				isPickingUp = false
			end, function()
				ClearPedTasks(playerPed)
				isPickingUp = false
